...
const Card = ({ chord, onClick, selected, selectionOrder, uniqueId }) => {
  return (
    <button
      className={`m-1 group border-4 border-primary-100 ${selected ? "bg-primary-100" : "bg-primary-50 "} rounded-lg cursor-pointer`}
      onClick={onClick}
    >
      <div className={`relative flex rounded-t-md items-center justify-between text-center py-4 ${selected ? "bg-primary-400" : "bg-primary-200"}`}>
        <p
          ...
          font-extrabold rounded-full text-primary-600 ${selected ? "bg-primary-100" : "invisible"}`}
        >
          {getSelectionNumber(uniqueId, selectionOrder)}
        </p>
        <h3 className={`mx-auto leading-none sm:text-2xl font-black ${selected ? "text-primary-50" : "text-accent-600"}`}>{chord.name}</h3>
      </div>
