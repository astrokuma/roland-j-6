...
const Card = ({ chord, onClick, selected, selectionOrder, uniqueId }) => {
  return (
    <button
      className={`m-1 group ${selected ? "bg-primary-200" : "bg-primary-100"} rounded-lg cursor-pointer`}
      onClick={onClick}
    >
      <div className={`relative flex rounded-t-md items-center justify-between text-center py-4 ${selected ? "bg-primary-400" : "bg-primary-200"}`}>
        <p
          ...
          font-extrabold rounded-full text-primary-600 ${selected ? "bg-primary-200" : "invisible"}`}
        >
