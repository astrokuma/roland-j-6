...
const Card = ({ chord, onClick, selected, selectionOrder, uniqueId }) => {
  return (
    <button
      ...
      onClick={onClick}
    >
      <div className={`relative flex items-center justify-between text-center py-4 rounded-t-lg border-b-4 border-accent ${selected ? "bg-accent" : "bg-primary"}`}>
        <p
        ...
        <h3 className={`mx-auto leading-none sm:text-2xl font-black ${selected ? "text-primary" : "text-accent"}`}>{chord.name}</h3>
      </div>
